...
#pip install -r requirements.txt  # replit
...
#影格擷取與合成影片  (原始render錯誤f"RCaop{uuid.uuid4()}.mp4"
def cap_to_video(save_path, output_path, target_fps=12): #.mp4", target_fps=12):#"output.mp4", target_fps=12): #.mp4", target_fps=12):
    ...
    while cap.isOpened():
        ...
        if count % frame_interval == 0:
            frame = (frame - np.min(frame)) * (255/(np.max(frame)-np.min(frame)))
...
async def upload_video(file: UploadFile = File(...), background_tasks: BackgroundTasks = BackgroundTasks()): #, background_task:BackgroundTasks = BackgroundTasks()):
    save_path = f"RCaop{uuid.uuid4()}.mp4"
    output_path = f"RCaop{uuid.uuid4()}.mp4"
    ...
    save_path = cap_to_video(save_path, output_path)
...
幫我修改commit change的內文
標題:update video transform
內容:i rebuild the video transform logic in order to fit the online deploy limit, this time i use imageio to try whether render and replit can run success or not.
'''
